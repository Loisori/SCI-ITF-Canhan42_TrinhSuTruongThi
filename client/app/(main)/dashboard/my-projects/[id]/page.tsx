...
export default function MyProjectMilestonesPage() {
  ...
  const [milestones, setMilestones] = useState<
    { title: string; percentage: number; stage: number }[]
  >([]);
  ...
  const handleSaveMilestones = async () => {
    ...
    try {
      ...
      setToast({
        type: "error",
        message: err.response?.data?.message || "Lưu cấu hình thất bại.",
      });
    } finally {
      setSaving(false);
    }
  ...
  const handleStartVoting = async (milestoneId: number) => {
    ...
    try {
      ...
      setToast({
        type: "error",
        message: err.response?.data?.message || "Không thể bắt đầu bình chọn.",
      });
    } finally {
      setSaving(false);
    }
  ...
  const handleProofSelected = async (url: string) => {
    ...
    try {
      ...
      setToast({
        type: "error",
        message: err.response?.data?.message || "Không thể nộp minh chứng.",
      });
    }
  };
