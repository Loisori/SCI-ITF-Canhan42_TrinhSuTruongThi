          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm mb-10">
             ...
             <div className="space-y-4">
                {project.milestones && project.milestones.length > 0 ? project.milestones.map((m: any, idx) => (
                   <div key={idx} className={`p-5 rounded-xl border flex justify-between gap-6 overflow-hidden relative ${
                      m.status === 'disbursed' ? 'bg-green-50/50 border-green-200' : 
                      ...
                      <div className="flex gap-4 items-start relative z-10 w-full">
                         <div className={`size-12 flex items-center justify-center font-bold text-lg rounded-full shrink-0 ${
                            m.status === 'disbursed' ? 'bg-green-600 text-white' : 'bg-white shadow text-slate-700'
                         }`}>
                            {idx + 1}
                         </div>
                         <div className="w-full">
                            ...
                            <div className="mt-1 flex items-center gap-3">
                               <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase ${
                                  m.status === 'disbursed' ? 'bg-green-100 text-green-700' : 
                            ...
                            {m.proofUrl && m.status !== 'uploading_proof' && (
                               <div className="mt-4 opacity-80 flex items-center gap-2">
                                  <CircleCheck className="text-[16px] text-green-600" />
