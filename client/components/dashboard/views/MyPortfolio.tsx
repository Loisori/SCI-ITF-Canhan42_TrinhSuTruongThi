            ...
            <div className="flex flex-col md:flex-row justify-between gap-6">
              <div className="flex gap-4 min-w-0">
                {inv.project?.thumbnailUrl ? (
                  <img src={inv.project.thumbnailUrl} alt="" className="w-24 h-16 rounded-xl object-cover shrink-0" />
                ) : (
                  <div className="w-24 h-16 rounded-xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center shrink-0">
                    <span className="material-symbols-outlined text-slate-300">image</span>
                  </div>
                )}
                <div className="min-w-0">
                  <h3 className="font-bold text-slate-900 dark:text-white truncate text-base">{inv.project?.title}</h3>
