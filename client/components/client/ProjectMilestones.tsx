...
export default function ProjectMilestones({
  ...
  const handleUploadProof = async (milestoneId: number) => {
    ...
    try {
      ...
      setToast({
        type: "error",
        message: err.response?.data?.message || "Lỗi cập nhật bằng chứng",
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleStartVoting = async (milestoneId: number) => {
    ...
    try {
      ...
      setToast({
        type: "error",
        message: err.response?.data?.message || "Lỗi bắt đầu bình chọn",
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleVoteRest = async (milestoneId: number, isApprove: boolean) => {
    ...
    try {
      ...
      setToast({
        type: "error",
        message: err.response?.data?.message || "Lỗi gửi bình chọn",
      });
    } finally {
      setLoading(false);
    }
