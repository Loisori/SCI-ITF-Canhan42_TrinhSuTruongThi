...
export class MilestonesService {
  ...
  async createOrUpdateMilestones(
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      const project = await projectRepo.findOne({ where: { id: projectId, ownerId } });
      if (!project) throw new NotFoundException('Project not found');
      if (project.status !== ProjectStatus.PENDING && project.status !== ProjectStatus.FUNDING) {
        throw new BadRequestException('Cannot update milestones after funding ends');
      }

      await milestoneRepo.delete({ projectId });

      const entities = milestonesData.map(m => milestoneRepo.create({
        projectId,
        title: m.title,
        percentage: m.percentage,
        stage: m.stage,
